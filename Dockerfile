...
RUN mkdir -p /app /runner/repositories /runner/hooks /runner/deployments \
...
COPY build/hooks/post-receive /runner/hooks/post-receive
...
RUN chmod +x /usr/local/bin/backplane-ssh /runner/hooks/post-receive /usr/local/bin/docker-entrypoint.sh
